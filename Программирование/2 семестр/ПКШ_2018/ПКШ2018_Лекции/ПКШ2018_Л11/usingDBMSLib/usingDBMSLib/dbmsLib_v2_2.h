//файл dbmsLib_v2_2.h
#pragma once
#ifndef _dbmsLib_v2_2_
#define _dbmsLib_v2_2_
#include <iostream>
#include <fstream>
#include <strstream>
#include <iomanip>
...
//-----------------класс DBDate----------------------------
  ...
  {	
	...
	public:
		...
		DBDate():day(1),month(1),year(1){};//конструктор по умолчанию
...
//Condition - перечисление условий сравнения:
enum Condition{
	Undefined,
	Equal,
	NotEqual,
...
//===============Абстрактный базовый класс DBTable================
class DBTable{
public:
	DBTable(){};
...
//в таблице может быть несколько записей с одинаковым значением вторичного ключа:
	...
	//относительное имя файла: fileName="..\\"+dbName+"\\"+tableName+".bin"|".txt"
	...
	virtual void PrintTable(int screenWidth)=0;//screenWidth - ширина экрана
	virtual Row CreateRow()=0;
	virtual void AddRow(Row row,int index)=0;
//выборка из таблицы строк, в которых значение поля colName и значение value 
	//удовлетворяют условию cond:
