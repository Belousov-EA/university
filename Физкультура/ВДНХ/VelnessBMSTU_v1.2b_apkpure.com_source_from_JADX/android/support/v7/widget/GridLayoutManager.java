...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        if (flexibleInOtherDir) {
            ...
            for (i = 0; i < count; i++) {
                view = this.mSet[i];
                measureChild(view, Ints.MAX_POWER_OF_TWO, true);
        ...
        for (i = 0; i < count; i++) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view) != maxSize) {
                ...
                if (this.mOrientation == 1) {
                    wSpec = LayoutManager.getChildMeasureSpec(totalSpaceInOther, Ints.MAX_POWER_OF_TWO, horizontalInsets, lp.width, false);
                    hSpec = MeasureSpec.makeMeasureSpec(maxSize - verticalInsets, Ints.MAX_POWER_OF_TWO);
                } else {
                    wSpec = MeasureSpec.makeMeasureSpec(maxSize - horizontalInsets, Ints.MAX_POWER_OF_TWO);
                    hSpec = LayoutManager.getChildMeasureSpec(totalSpaceInOther, Ints.MAX_POWER_OF_TWO, verticalInsets, lp.height, false);
                }
