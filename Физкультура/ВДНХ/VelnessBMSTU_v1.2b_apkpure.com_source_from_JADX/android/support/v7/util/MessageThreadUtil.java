...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C01831();
            ...
            class C01831 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C05331.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C01841();
            ...
            class C01841 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C05342.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C05342.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C05342.this.mQueue.removeMessages(2);
                                    C05342.this.mQueue.removeMessages(3);
                        ...
                        C05342.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
