...
public class SlidingPaneLayout extends ViewGroup {
    ...
    static class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C05221());
        boolean isOpen;

        static class C05221 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C05221() {
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode != 1073741824) {
            if (!isInEditMode()) {
                ...
                if (widthMode == 0) {
                    widthSize = HttpStatusCodes.STATUS_CODE_MULTIPLE_CHOICES;
                }
            }
            ...
            if (!isInEditMode()) {
                ...
                heightSize = HttpStatusCodes.STATUS_CODE_MULTIPLE_CHOICES;
            }
        }
        int layoutHeight = 0;
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.width == -2) {
                    ...
                    childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, Ints.MAX_POWER_OF_TWO);
                } else {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width, Ints.MAX_POWER_OF_TWO);
                }
                if (lp.height == -2) {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                } else if (lp.height == -1) {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Ints.MAX_POWER_OF_TWO);
                } else {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(lp.height, Ints.MAX_POWER_OF_TWO);
                }
                ...
                if (child.getVisibility() != 8) {
                    ...
                    if (child.getVisibility() != 8) {
                        ...
                        if (!canSlide || child == this.mSlideableView) {
                            if (lp.weight > 0.0f) {
                                if (lp.width != 0) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(child.getMeasuredHeight(), Ints.MAX_POWER_OF_TWO);
                                } else if (lp.height == -2) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                                } else if (lp.height == -1) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Ints.MAX_POWER_OF_TWO);
                                ...
                                if (canSlide) {
                                    ...
                                    childWidthSpec = MeasureSpec.makeMeasureSpec(newWidth, Ints.MAX_POWER_OF_TWO);
                                    if (measuredWidth != newWidth) {
                                        child.measure(childWidthSpec, childHeightSpec);
                                    }
                                } else {
                                    child.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), Ints.MAX_POWER_OF_TWO), childHeightSpec);
                                }
                            }
                            ...
                            child.measure(MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, Ints.MAX_POWER_OF_TWO), childHeightSpec);
                        }
                    }
                }
            }
