...
public final class zzfjj {
    ...
    public final int zzcwi() throws IOException {
        ...
        int i = zzcwr & TransportMediator.KEYCODE_MEDIA_PAUSE;
        byte zzcwr2 = zzcwr();
        if (zzcwr2 >= (byte) 0) {
            return i | (zzcwr2 << 7);
        }
        i |= (zzcwr2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 7;
        zzcwr2 = zzcwr();
        if (zzcwr2 >= (byte) 0) {
            return i | (zzcwr2 << 14);
        }
        i |= (zzcwr2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 14;
        zzcwr2 = zzcwr();
        if (zzcwr2 >= (byte) 0) {
            return i | (zzcwr2 << 21);
        }
        i |= (zzcwr2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 21;
        zzcwr2 = zzcwr();
        i |= zzcwr2 << 28;
        if (zzcwr2 >= (byte) 0) {
            return i;
