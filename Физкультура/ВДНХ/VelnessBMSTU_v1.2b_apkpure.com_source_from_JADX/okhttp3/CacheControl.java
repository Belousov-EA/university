    ...
    public static CacheControl parse(Headers headers) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!name.equalsIgnoreCase(HttpHeaders.CACHE_CONTROL)) {
                if (name.equalsIgnoreCase(HttpHeaders.PRAGMA)) {
            ...
            while (pos < value.length()) {
                String parameter;
                int tokenStart = pos;
                pos = okhttp3.internal.http.HttpHeaders.skipUntil(value, pos, "=,;");
                String directive = value.substring(tokenStart, pos).trim();
                if (pos == value.length() || value.charAt(pos) == ',' || value.charAt(pos) == ';') {
                    pos++;
                    parameter = null;
                } else {
                    pos = okhttp3.internal.http.HttpHeaders.skipWhitespace(value, pos + 1);
                    int parameterStart;
